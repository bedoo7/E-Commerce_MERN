...
async function dateRangeDays(days: number) {
	const end = new Date();
...
export const getDashboardAnalytics = async () => {
	try {
		...
		const prevMonthRevResult = await orderModel.aggregate([
		...
		const ordersThisWeekCount = await orderModel.countDocuments({ createdAt: { $gte: weekStart } });
		const ordersThisMonthCount = await orderModel.countDocuments({ createdAt: { $gte: monthStart } });

		const totalOrdersAll = await orderModel.countDocuments({});

		// Orders by status (from orders)
		const ordersByStatusResult = await orderModel.aggregate([
		...
		ordersByStatusResult.forEach((s: any) => {
		...
		const activeProductsCount = await productModel.countDocuments({ stock: { $gt: 0 } });
		const outOfStockCount = await productModel.countDocuments({ stock: { $lte: 0 } });
		const lowStockCount = await productModel.countDocuments({ stock: { $lte: 5, $gt: 0 } });
		...
		const worstSelling = await orderModel.aggregate([
			{ $match: { status: { $ne: "cancelled" } } },
			{ $unwind: "$orderItems" },
			{
				$group: {
		...
		const soldProductNamesResult = await orderModel.aggregate([
			{ $match: { status: { $ne: "cancelled" } } },
			{ $unwind: "$orderItems" },
			{ $group: { _id: "$orderItems.productTitle" } },
		]);
		const soldNames = soldProductNamesResult.map((p: any) => p._id);
		const neverOrderedProducts = await productModel.find({ name: { $nin: soldNames } }).limit(10).lean();

		// Low Stock Products (detailed)
		const lowStockDetails = await productModel.find({ stock: { $lte: 5, $gt: 0 } }).limit(10).lean();

		// Out of Stock Products (detailed)
		const outOfStockDetails = await productModel.find({ stock: { $lte: 0 } }).limit(10).lean();

		// Highest Revenue Products
		const highestRevenue = await orderModel.aggregate([
			{ $match: { status: { $ne: "cancelled" } } },
			{ $unwind: "$orderItems" },
			{
				$group: {
		...
		const recentlyAdded = await productModel.find().sort({ createdAt: -1 }).limit(10).lean();

		// ── Inventory ────────────────────────────────────
		const inventoryLow = await productModel.find({ stock: { $lte: 5, $gt: 0 } }).select("name stock category").lean();
		const inventoryOutOfStock = await productModel.find({ stock: { $lte: 0 } }).select("name stock category").lean();

		// ── Customer Analytics ───────────────────────────
		const totalUsersCount = await userModel.countDocuments();
		const newUsersTodayCount = await userModel.countDocuments({ createdAt: { $gte: todayStart } });
		const newUsersThisMonthCount = await userModel.countDocuments({ createdAt: { $gte: monthStart } });
		...
		const highestSpending = await orderModel.aggregate([
			{ $match: { status: { $ne: "cancelled" }, userId: { $exists: true } } },
		...
		const mostOrders = await orderModel.aggregate([
			{ $match: { status: { $ne: "cancelled" }, userId: { $exists: true } } },
			{ $group: { _id: "$userId", orderCount: { $sum: 1 }, totalSpent: { $sum: "$totalAmount" }, lastOrderDate: { $max: "$createdAt" } } },
			{ $sort: { orderCount: -1 } },
			{ $limit: 10 },
		...
		const customerIds = [
			...returningCustomers.map((c: any) => c._id),
			...highestSpending.map((c: any) => c._id),
			...mostOrders.map((c: any) => c._id),
		];
		const uniqueCustomerIds = [...new Set(customerIds.map((id: any) => id.toString()))];
		const customerDetails = await userModel.find({ _id: { $in: uniqueCustomerIds.map((id: string) => new mongoose.Types.ObjectId(id)) } }).select("firstName lastName email").lean();
		const customerMap: Record<string, any> = {};
		(customerDetails as any[]).forEach((c: any) => {
	...
	const inactiveCouponsCount = couponStats[0]?.inactiveCoupons || 0;
		...
		// Most Used Coupons
		const mostUsedCoupons = await couponModel.find({ usedCount: { $gt: 0 } }).sort({ usedCount: -1 }).limit(10).lean();

		// Never Used Coupons
		const neverUsedCoupons = await couponModel.find({ usedCount: 0 }).limit(10).lean();

		// Total Discounts Given
		const totalDiscountsResult = await orderModel.aggregate([
		...
		const ordersByCategory = await orderModel.aggregate([
			{ $match: { status: { $ne: "cancelled" } } },
			{ $unwind: "$orderItems" },
			{
				$lookup: {
		...
		const avgProductsResult = await orderModel.aggregate([
			{ $match: nonCancelled },
		...
		return {
			...
			products: {
				...
				topSelling: bestSelling.map((p: any) => ({
					name: p._id,
					image: p.productImage,
					totalQuantity: p.totalQuantity,
					totalRevenue: p.totalRevenue,
				})),
				bottomSelling: worstSelling.map((p: any) => ({
					name: p._id,
					image: p.productImage,
					totalQuantity: p.totalQuantity,
					totalRevenue: p.totalRevenue,
				})),
				neverOrdered: (neverOrderedProducts as any[]).map((p: any) => ({
					_id: p._id,
					name: p.name,
					imageUrl: p.imageUrl,
					category: p.category,
				...
				lowStockProducts: (lowStockDetails as any[]).map((p: any) => ({
					_id: p._id,
					name: p.name,
					imageUrl: p.imageUrl,
					category: p.category,
				...
				outOfStockProducts: (outOfStockDetails as any[]).map((p: any) => ({
					_id: p._id,
					name: p.name,
					imageUrl: p.imageUrl,
					category: p.category,
				...
				highestRevenue: highestRevenue.map((p: any) => ({
					name: p._id,
					image: p.productImage,
					totalRevenue: p.totalRevenue,
					totalQuantity: p.totalQuantity,
				})),
				recentlyAdded: (recentlyAdded as any[]).slice(0, 10).map((p: any) => ({
					_id: p._id,
					name: p.name,
					imageUrl: p.imageUrl,
					category: p.category,
			...
			inventory: {
				lowStock: (inventoryLow as any[]).map((p: any) => ({
					_id: p._id,
					name: p.name,
					category: p.category,
					stock: p.stock,
				...
				outOfStock: (inventoryOutOfStock as any[]).map((p: any) => ({
					_id: p._id,
					name: p.name,
					category: p.category,
					stock: p.stock,
			...
			customers: {
				...
				returningCustomers: (returningCustomers as any[]).length,
				highestSpending: (highestSpending as any[]).map((c: any) => ({
					userId: c._id,
					...(customerMap[c._id.toString()] || {}),
					totalSpent: c.totalSpent,
					orderCount: c.orderCount,
				})),
				mostOrders: (mostOrders as any[]).map((c: any) => ({
					userId: c._id,
					...(customerMap[c._id.toString()] || {}),
					totalSpent: c.totalSpent,
					orderCount: c.orderCount,
			...
			coupons: {
				...
				mostUsedCoupons: (mostUsedCoupons as any[]).map((c: any) => ({
					code: c.code,
					discountPercent: c.discountPercent,
					usedCount: c.usedCount,
					usageLimit: c.usageLimit,
				})),
				neverUsedCoupons: (neverUsedCoupons as any[]).map((c: any) => ({
					code: c.code,
					discountPercent: c.discountPercent,
				})),
			...
			categories: {
				revenueByCategory: (revenueByCategory as any[]).map((c: any) => ({
					category: c._id,
					revenue: c.revenue,
					orders: c.orders,
				})),
				ordersByCategory: (ordersByCategory as any[]).map((c: any) => ({
					category: c._id,
					orders: c.orders,
				})),
			...
			charts: {
				...
				weeklyRevenue: (weeklyRevenue as any[]).map((r: any) => ({
					label: `W${r._id.week} ${r._id.year}`,
					revenue: r.revenue,
					orders: r.orders,
				})),
				monthlyRevenue: (monthlyRevenue as any[]).map((r: any) => ({
					label: `${r._id.month}/${r._id.year}`,
					revenue: r.revenue,
					orders: r.orders,
				})),
				yearlyRevenue: (yearlyRevenue as any[]).map((r: any) => ({
					label: String(r._id.year),
					revenue: r.revenue,
					orders: r.orders,
				})),
				ordersOverTime: (ordersOverTime as any[]).map((r: any) => ({
					label: `${r._id.month}/${r._id.day}/${r._id.year}`,
					orders: r.orders,
				})),
			},
